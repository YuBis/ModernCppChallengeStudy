...
bool IPv4::isBiggerThan(IPv4* other)
{
	return std::tie(octet_arr_.at(0), octet_arr_.at(1), octet_arr_.at(2), octet_arr_.at(3))
		> std::tie(other->octet_arr_.at(0), other->octet_arr_.at(1), other->octet_arr_.at(2), other->octet_arr_.at(3));
}
...
void PrintRange(IPv4* begin_addr, IPv4* end_addr)
{
	if (begin_addr->isBiggerThan(end_addr))
		std::swap(begin_addr, end_addr);

	IPv4* print_addr = begin_addr;
	print_addr->add_one();

	while (end_addr->isBiggerThan(print_addr))
	{
		print_addr->PrintThis();
		print_addr->add_one();
	}
}
...
int main()
{
	...
	InsertAddress(input[0]);
	InsertAddress(input[1]);

	PrintRange(item_vec_.at(0), item_vec_.at(1));
